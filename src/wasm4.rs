...
pub fn set_pixels<F: Fn(i32, i32, u8) -> u8>(get_color: F) {
    const BITS_PER_PIXEL: i32 = 2;
    const PIXELS_PER_BYTE: i32 = 8 / BITS_PER_PIXEL;
    for y in 0..SCREEN_SIZE {
        for x in 0..SCREEN_SIZE {
            let pixel_number = y * SCREEN_SIZE + x;
            let index = (pixel_number / PIXELS_PER_BYTE) as usize;
            let bit_offset = ((pixel_number % PIXELS_PER_BYTE) * BITS_PER_PIXEL) as u8;

            let prev_val;
            let prev_color;
            unsafe {
                prev_val = (*FRAMEBUFFER)[index];
                prev_color = (prev_val >> bit_offset) & 0b11;
            }

            let color = get_color(x, y, prev_color);

            unsafe {
                let cleared_value = prev_val & !(0b11 << bit_offset);
                let new_value = cleared_value | (color << bit_offset);
                (*FRAMEBUFFER)[index] = new_value;
            }
        }
    }
}
